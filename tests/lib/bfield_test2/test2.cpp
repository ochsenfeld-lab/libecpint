...
int main(int argc, char* argv[]) {
	...
	ECPIntegral ecpint(4, 4, 1); 
	std::array<TwoIndex<double>, 3> results; 
	std::vector<double> flat_result;
	
	ecpint.compute_shell_pair_derivative_Bfield(newU, shellA, shellA, results);
	for (int i = 0; i < 3; i++) {
		for (auto v : results[i].data) 
			flat_result.push_back(v); 
	}
	
	ecpint.compute_shell_pair_derivative_Bfield(newU, shellA, shellB, results);
	for (int i = 0; i < 3; i++) {
		for (auto v : results[i].data) 
			flat_result.push_back(v); 
	}
	
	ecpint.compute_shell_pair_derivative_Bfield(newU, shellB, shellB, results);
	for (int i = 0; i < 3; i++) {
		for (auto v : results[i].data) 
			flat_result.push_back(v); 
	}
	...
	return check_file<double>("bfield_test2.output", flat_result, 1e-5, 1e-8);
}
